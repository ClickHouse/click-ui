...
- 5cd6b3e: Adds a new `DateTimePicker` component for selecting date and time ranges with precision control. This component combines calendar-based date selection with time input fields, supporting both predefined time ranges and custom selections.
...
- 6ddd8ca: The Click-UI source code has several circular dependencies that must be resolved.
...
- 2368781: The team should have full control over the Public API to manage which resources are available for use in consumer applications. Previously, consumer applications had unrestricted access to internal resources, which is undesirable.
...
- b0996fc: Given the request [813](https://github.com/ClickHouse/click-ui/issues/813), the following provides support for root colour theme attributes.
...
- 2f34587: Adds a new `isResponsive` prop to the Table component.
...
- 5ded77a: Enable keyboard date picker selection
...
- 97bafb7: Enforce generic type annotation style for arrays via ESLint
...
- 7526692: Introduces click-ui's own `DialogProps` and `DialogTriggerProps` types, replacing direct Radix UI type re-exports. This decouples the public API from internal implementation details.
...
- 3374d96: Adds an `openDirection` prop to `DateRangePicker` that controls which side the custom date range calendar opens. This is useful when the component is positioned near the right edge of the viewport, allowing the calendar to open on the left side to prevent overflow.
...
- 8668989: Resolve component path redundancy and allow public API encapsulation.
...
- 7c9c75d: Removes the ESLint/TSLint rule that enforced arrays to be typed using generic syntax (Array<T>) instead of the shorthand array syntax (T[]).
...
- cf1d08b: Deprecated StyledLinkProps and linkStyles in the public API. These will be removed in a future release to prevent leaking styled-components implementation details, e.g. $size and $weight transient props in the Public API
...
- 976d373: Restore changes lost in PR 841-845 merge conflict resolution.
...
- 75f675c: Add circular dependency check to prevent and detect circular import cycles that can cause build issues, runtime errors, and bundle size problems.
