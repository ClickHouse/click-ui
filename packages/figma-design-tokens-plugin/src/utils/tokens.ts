...
export function inferScopes(
  ...
  if (type === 'FLOAT' || type === 'number') {
    console.log(`DEBUG inferScopes - Checking FLOAT/number: "${normalizedName}"`);
    ...
    if (normalizedName.includes('font/size') || normalizedName.includes('font.size')) {
      console.log(`DEBUG inferScopes - Matched FONT_SIZE for "${normalizedName}"`);
      return ['FONT_SIZE'];
    }

    if (
      normalizedName.includes('font/weight') ||
      normalizedName.includes('font.weight')
    ) {
      console.log(`DEBUG inferScopes - Matched FONT_WEIGHT for "${normalizedName}"`);
      return ['FONT_WEIGHT'];
    }
    ...
    if (
      ...
      console.log(`DEBUG inferScopes - Matched LINE_HEIGHT for "${normalizedName}"`);
      return ['LINE_HEIGHT'];
    }

    if (normalizedName.includes('radius') || normalizedName.includes('corner')) {
      console.log(`DEBUG inferScopes - Matched CORNER_RADIUS for "${normalizedName}"`);
    ...
    if (
      ...
      console.log(`DEBUG inferScopes - Matched WIDTH_HEIGHT for "${normalizedName}"`);
      return ['WIDTH_HEIGHT'];
    }
    ...
    if (normalizedName.includes('opacity')) {
      console.log(`DEBUG inferScopes - Matched OPACITY for "${normalizedName}"`);
      return ['OPACITY'];
    }
    ...
    console.log(`DEBUG inferScopes - Default ALL_SCOPES for "${normalizedName}"`);
    return ['ALL_SCOPES'];
  }
...
export async function createCollection(
  ...
  if (existingCollection) {
    ...
    if (withModes) {
      ...
      if (lightMode === undefined && existingCollection.modes[0]?.name === 'Mode 1') {
...
export function createToken(
  ...
  console.log(
    ...
    existingVariables
      ? `defined (${Object.keys(existingVariables).length} vars)`
      : 'undefined'
  );
  ...
  if (existingVariables) {
    ...
    if (existingVariables[name]) {
      ...
      console.log(`DEBUG createToken - Existing modes for "${name}":`, existingModeIds);
      console.log(`DEBUG createToken - Current import modeId: ${modeId}`);

      if (existingModeIds.length > 0) {
        const targetModeId = existingModeIds.includes(modeId)
          ? modeId
          : existingModeIds[0]!;
        console.log(`DEBUG createToken - Updating value for mode ${targetModeId}`);

        // Handle mode values (light/dark) when updating existing tokens
        if (modeIds && modeValues) {
          console.log(
            `DEBUG createToken - Has modeIds and modeValues, updating both modes`
          );
          if (modeValues.light !== undefined && existingModeIds.includes(modeIds.light)) {
            console.log(
              `DEBUG createToken - Setting light mode (${modeIds.light}) to:`,
              modeValues.light
            );
            token.setValueForMode(modeIds.light, modeValues.light);
          } else {
            console.log(
              `DEBUG createToken - Setting light mode (${modeIds.light}) to base value:`,
              value
            );
          ...
          if (
            modeIds.dark &&
            modeValues.dark !== undefined &&
            existingModeIds.includes(modeIds.dark)
          ) {
            console.log(
              `DEBUG createToken - Setting dark mode (${modeIds.dark}) to:`,
              modeValues.dark
            );
        ...
        console.error(`DEBUG createToken - No modes found for existing token "${name}"`);
      }
      ...
      if (scopes) {
        ...
        const scopesChanged =
          JSON.stringify(currentScopes.sort()) !== JSON.stringify(scopes.sort());
        if (scopesChanged) {
          try {
            (token as VariableWithScopes).scopes = scopes as VariableScope[];
            console.log(`DEBUG createToken - Updated scopes for "${name}" to:`, scopes);
          } catch (e) {
            console.error(
      ...
      console.log(`DEBUG createToken - Successfully updated existing token "${name}"`);
      return token;
    }
    ...
    if (existingVariables[dotName]) {
      ...
      console.log(`DEBUG createToken - Successfully updated existing token "${dotName}"`);
      return token;
    }
  }
...
export async function getExistingVariables(): Promise<Record<string, Variable>> {
...
export function traverseToken({
  ...
  console.log(
    `DEBUG traverseToken - ENTER: key="${key}", hasValue=${object.$value !== undefined}, type=${object.$type || type}, isPrimitivesFile=${isPrimitivesFile}`
  );
  ...
  if (object.$value !== undefined) {
    ...
    if (isAlias(value)) {
      const valueKey = value.toString().trim().replace(/\./g, '/').replace(/[{}]/g, '');

      const allTokens = { ...existingVariables, ...tokens };
      console.log(
        `DEBUG traverseToken - Alias check: "${finalKey}" -> "${valueKey}", found=${!!allTokens[valueKey]}`
      );
      ...
      if (allTokens[valueKey]) {
        ...
        if (modeIds && modeExtensions) {
          ...
          if (lightUnresolved || darkUnresolved) {
            ...
            console.log(
              `DEBUG traverseToken - Creating mode-aware alias token: "${finalKey}"`
            );
            ...
            console.log(
              `DEBUG traverseToken - SUCCESS: Created mode-aware alias token "${finalKey}"`
            );
          }
        } else {
          // Token is mode-agnostic but collection has modes - pass modeIds to set value for ALL modes
          console.log(
            `DEBUG traverseToken - Creating mode-agnostic alias token: "${finalKey}" with modeIds`
          );
          tokens[finalKey] = createVariableAlias(
            collection,
            modeId,
          ...
          console.log(
            `DEBUG traverseToken - SUCCESS: Created mode-agnostic alias token "${finalKey}"`
          );
        }
      } else {
        console.log(
          `DEBUG traverseToken - Adding to aliases: "${finalKey}" -> "${valueKey}" (target not found yet)`
        );
        aliases[finalKey] = {
          key: finalKey,
          type: resolvedType,
      ...
      const description =
        object.$description || generateDescription(finalKey, String(value), 'color');
      console.log(`DEBUG - About to createToken for "${finalKey}" with scopes:`, scopes);
      ...
      if (modeIds && modeExtensions) {
        ...
        const darkValue = resolveModeValue(modeExtensions.dark, resolvedType, allTokens);
      ...
      const description =
        object.$description || generateDescription(finalKey, numericValue, 'number');
      ...
      console.log(
        `DEBUG traverseToken - Creating number/dimension token: "${finalKey}" with value ${numericValue}`
      );
      tokens[finalKey] = createToken(
        collection,
        modeId,
      ...
      console.log(
        `DEBUG traverseToken - SUCCESS: Created number/dimension token "${finalKey}"`
      );
    } else {
      console.log(
        `DEBUG traverseToken - unsupported type for "${finalKey}":`,
        resolvedType,
        object
      );
    }
    ...
    console.log(
      `DEBUG traverseToken - Recursing into "${finalKey}" with ${childKeys.length} children: ${childKeys.slice(0, 5).join(', ')}${childKeys.length > 5 ? '...' : ''}`
    );
    ...
    console.log(
      `DEBUG traverseToken - SKIPPING "${finalKey}": not an object and no $value`
    );
  }
...
export async function processAliases({
  ...
  console.log('DEBUG - Available new tokens:', Object.keys(tokens).slice(0, 10));
          ...
          if (lightToken || darkToken) {
            resolvedModeValues = {
              light: lightToken
                ...
                : undefined,
              dark: darkToken ? { type: 'VARIABLE_ALIAS', id: darkToken.id } : undefined,
            };
          }
        }
