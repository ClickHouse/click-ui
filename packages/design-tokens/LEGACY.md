...
import type { CUIThemeType, UseThemeParams } from '@clickhouse/design-tokens/legacy/hooks';
```
...
import type { ThemeName, Theme, InitCUIThemeScriptProps } from '@clickhouse/design-tokens/legacy/theme';
```
...
function Component() {
  return (
    <div style={{ color: 'var(--cui-color-foreground-default)' }}>
      Content
    </div>
  );
}
