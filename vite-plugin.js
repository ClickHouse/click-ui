...
function generateCSSFromConfig(config) {
    ...
    if (config.theme || config.systemModeOverrides?.light) {
        const lightVars = generateVariables({ ...config.theme, ...config.systemModeOverrides?.light }, prefix);
        css += `@media (prefers-color-scheme: light) {\n  :root {\n${lightVars}  }\n}\n\n`;
        css += `:root[data-theme="light"] {\n${lightVars}}\n\n`;
    }
    // Dark theme
    if (config.systemModeOverrides?.dark) {
        const darkVars = generateVariables({ ...config.theme, ...config.systemModeOverrides?.dark }, prefix);
