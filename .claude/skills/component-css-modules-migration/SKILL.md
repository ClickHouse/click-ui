...
- **Exported, polymorphic styled wrappers (`as` prop) must keep their API.** When a styled-component is *exported* and consumed polymorphically — `export const SidebarItemWrapper = styled.div\`…\`` rendered elsewhere as `<SidebarItemWrapper as={Collapsible.Header} $collapsible $level={1} $type="main" />` — other components depend on both the export and its transient-prop contract. Do **not** inline the wrapper's styles onto each call site or delete the export to "simplify"; that's a public-API change and breaks every `as=` consumer. Instead convert the wrapper into a plain component that (a) keeps the same name and export, (b) accepts the same `$`-prefixed props plus `className` and `as`, (c) destructures the `$` props out so they never reach the DOM (styled-components filtered them for free; `cn()` does not), and (d) forwards to the `as` target:
  ...
  - **Cascade precedence between the two class sets.** When the wrapper renders `as` a target that also has its own class, *both* classes land on one element and any property both define (the target often resets `display`/`color`/`font`/`cursor: inherit`) is resolved by **CSS-module bundle order**, not by which feels "more specific" — so the winner flips unpredictably between the standalone wrapper and the composed one. styled-components masked this via insertion order; CSS Modules makes it explicit. If the snapshot shows the target's reset winning (e.g. the title rendered in the inherited font instead of its own), raise the wrapper's *conflicting* declarations to a doubled-class selector (`.wrapper.wrapper { … }`, specificity `(0,2,0)`) so they beat the target's single class deterministically. **Keep the box metrics and every `--var` default at single-class specificity** — putting custom-property defaults in the doubled rule makes them outrank the single-class `_modifier` classes that override those vars (e.g. `wrapper_collapsible_true` setting `--pad-left: 0`), silently disabling the modifier. Only the handful of properties that literally collide with the `as` target belong in the doubled rule. (Caught migrating `SidebarNavigationTitle` rendered `as={Collapsible.Trigger}`.)
